...
@section('content')
    ...
    <!-- Content Header (Page header) -->
        <section class="content-header">
            <a href="{{route('projects.list')}}" style="text-decoration: underline; padding-bottom: 15px; display:block"><i
            ...
            <div class="row opt1">
                <div class="col-xs-12 col-md-6">
                    <input type="text" class="form-control mb-o1 mt-o1" id="search" onkeyup="search()"
                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach($users as $u)
                                    <tr>
                                        ...
                                        <td width="10%">
                                            <button class="btn btn-primary {{$hidd}}" id="add{{$u->id}}"
                                                    name="{{$u->id}}"><i class="fa fa-user-plus"></i></button>
                                        </td>
                                        <td width="10%">
                                            <button class="btn btn-danger {{$oshow}}" id="remove{{$u->id}}"
                                                    name="{{$u->id}}"><i class="fa fa-user-times"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '[id*="add"]', function () {
                ...
                //AJAX
                $.ajax({
                    url: '{{ route('api.projects.member.add') }}',
                    method: 'get',
                    data: {
                        'addValue': $addValue,
                        'p_id': $p_id
            ...
            $(document).on('click', '[id*="remove"]', function () {
                ...
                //AJAX
                $.ajax({
                    url: '{{route('api.projects.member.del')}}',
                    method: 'get',
                    data: {
